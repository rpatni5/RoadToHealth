...
namespace RTH.Business.Services
{
    public static class TwitterService
    {
        ...
        public static string PostTweet(string message, string url, TwitterCredentials credentials, IDictionary<string, string> parameters)
        {
            parameters.Remove("oauth_timestamp");
            parameters.Remove("screen_name");
            parameters.Remove("oauth_nonce");
            parameters.Remove("oauth_version");
            parameters.Remove("oauth_signature_method");
            parameters.Remove("status");
            ...
            string encodedMessage = Uri.EscapeDataString(message); // unencoded value is to be passed here
        ...
        public static ShareData GetSharingData(string userId, string keyString, string accessToken)
        {
            return Http.ExecuteGET<SocialSharingData>(string.Format("apis/get_facebook_share/{0}/{1}", userId, keyString), null, accessToken).data;
        }
        #endregion
    }
}
