...
namespace RTH.Windows.Views.UserControls
{
    ...
    public partial class SettingsView : ViewBase
    {
        ...
        private void btnLogout_Click(object sender, RoutedEventArgs e)
        {
            ...
            CleanUp();
            App.SharedValues.WindowMain.LoadView(Enums.ViewEnum.HomeView);
        }
        ...
        private void password_PasswordChanged(object sender, RoutedEventArgs e)
        {
            ...
            if (objSender.Name == txtPassword.Name)
            {
                ...
                Password.Visibility = count == 0 ? Visibility.Visible : Visibility.Collapsed;
            }
            else if (objSender.Name == txtNewPassword.Name)
            {
                btnNewPasswordClear.Visibility = count == 0 ? Visibility.Collapsed : Visibility.Visible;
                NewPassword.Visibility = count == 0 ? Visibility.Visible : Visibility.Collapsed;
            }
            ...
            else if (objSender.Name == txtConfirmPassword.Name)
            {
                ...
                ConfirmPassword.Visibility = count == 0 ? Visibility.Visible : Visibility.Collapsed;
            }
        ...
        private void AnimateEmail(double from, double to)
        {
            ...
            {
                Controls.QuealthMessageBox.ShowInformation(App.SharedValues.LanguageResource.Email_validation_sociallogin);
            }
        }
        ...
        private void tbtnFullName_Checked(object sender, RoutedEventArgs e)
        {

        }

        private void ErrorMessage_IsVisibleChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            if ((bool)e.NewValue)
            {
                Task.Delay(10000)
                     .ContinueWith(t => (this.ViewModel as SettingsViewModel).ErrorMessage = string.Empty);
            }
            scrMain.ScrollToTop();
        }

        private void tbtnPassword_Checked(object sender, RoutedEventArgs e)
        {
            AnimatePassword(0, 270);
        }

        private void tbtnPassword_Unchecked(object sender, RoutedEventArgs e)
        {
            AnimatePassword(270, 0);
        }
        private void AnimatePassword(double from, double to)
        {
            Storyboard qStoryboard = null;
            if (objUser.type == Configurations.NORMAL_USER_TYPE)
            {
                qStoryboard = new Storyboard();
                var da1 = new DoubleAnimation(from, to, System.TimeSpan.FromSeconds(0.3));
                Storyboard.SetTarget(da1, stackPassword);
                Storyboard.SetTargetProperty(da1, new PropertyPath("(StackPanel.Height)"));
                qStoryboard.Children.Add(da1);
                qStoryboard.Begin();
            }
