    ...
    public partial class HealthPlanView : ViewBase, ITutorialHost
    {
        ...
        private async void OnClick(object[] ve)
        {
            if (ve.Length > 3)
            {
                ...
                RTH.Windows.ViewModels.StrategyViewModel.NoOfObjectivesLeft = 5 - totalTargets;
                ...
                if (IsUserObjective)
                {
                    ...
                    {
                        grid.DataContext = await (this.ViewModel as HealthPlanViewModel).GetStrategy(id, ve[3]);
