...
namespace RTH.Business.Services
{
    ...
    {
        const int REQ_TIMEOUT_SECS = 100;
        ...
        public static T ExecuteGET<T>(string methodUrl, NameValueCollection queryString, string authToken = null)
        {
            ...
            using (var client = new HttpClient { Timeout = TimeSpan.FromSeconds(REQ_TIMEOUT_SECS) })
            {
        ...
        {
            using (HttpClient client = new HttpClient { Timeout = TimeSpan.FromSeconds(REQ_TIMEOUT_SECS) })
            {
            ...
            using (var client = new HttpClient())
            {
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        JObject json = JObject.Parse(a.Result);
                        var y= response.Content.ReadAsAsync<T>().Result;
                        return y;
                    }
                    else
                    {
                ...
                {
                    throw;
                    //new HttpException("There seems some internet connectivity issues. Please check and try again later");
                }
            }
        }
        ...
        private async static Task<T> ExecuteRequestAsync<T>(HttpClient client, RequestEnum requestType, string requestUri, HttpContent content = null)
        {
            ...
            {
                if (ex.InnerException is HttpException || ex.InnerException is HttpRequestException || ex.InnerException is WebException || ex.InnerException is TaskCanceledException)
                    return ProcessRequestOffline<T>(key, url, responseKey);
                else throw;
            }
            catch (Exception ex)
        ...
        private static T ProcessRequestOffline<T>(System.Collections.Generic.IDictionary<string, string> key, string url, string responseKey)
        {
            ...
            if (resp == null)
            {
                ...
                var cacheEx = new CachedObjectNotFoundException("You are not connected to internet. Please try again later.");
                cacheEx.Source = url;
