...
namespace RTH.Windows.ViewModels
{
    public class DashboardViewModel : ViewModelBase
    {
        ...
        public List<MotivationData> Motivations
        {
            get { return GetValue(() => this.Motivations); }
            set { SetValue(() => Motivations, value); }
        }
        public MotivationData CurrentMotivation
        {
            get { return GetValue(() => this.CurrentMotivation); }
            set { SetValue(() => CurrentMotivation, value); }
        }
        ...
        private void GetMotivation()
        {
            ...
            if (obj != null)
            {
                ...
                var mot = new List<MotivationData>();
                int i = 0;
                answer.ForEach(c =>
                {
                    mot.Add(new MotivationData()
                    {
                        RowNo = ++i,
                        IsPressed = i == 1 ? true : false,
                        MotivationText = c.answer.ToString(),
                    });
                });
                Motivations = mot;
            }
            ...
            {
                CurrentMotivation = Motivations.FirstOrDefault();
    ...
    public class MotivationData : NotifyBase
    {
        public int RowNo
        {
            get { return GetValue(() => this.RowNo); }
            set { SetValue(() => RowNo, value); }
        }
        public string MotivationText
        {
            get { return GetValue(() => this.MotivationText); }
            set { SetValue(() => MotivationText, value); }
        }
        public bool IsPressed
        {
            get { return GetValue(() => this.IsPressed); }
            set { SetValue(() => IsPressed, value); }
        }
    }
}
